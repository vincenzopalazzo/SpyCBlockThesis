...
La struttura finale del blocco, nella version attuale, può essere rappresentata nel modo indicato nella Figura \ref{tab:blockbitcoinc}
...
Come descritto nel capitolo di \ref{sec:transazioniBitcoin}, esse rappresentano l’elemento fulcro del sistema; nel corso degli anni, gli sviluppatori di Bitcoin core hanno dovuto confrontarsi con problemi nell'implementazione originale delle transazioni: Uno di essi è conosciuto come problema di malleabilità: {\it durante il processo di verifica di una transazione era possibile alterare l’id (txId) ed in particolare, la malleabilità della transazione coinvolge le firme nello script di sblocco (scriptSig) e ciò comporta delle irregolarità nel protocollo Bitcoin: \/}
...
Come i timelock assoluti, i timelock relativi vengono espressi sia a livello di script che a livello di transazion;, per fare ciò viene utilizzato il campo nSequence all’interno della transazione di input.
